...
func (svc *Service)GetArticle(param *ArticleRequest) (*Article,error) {
	...
	tag,err := svc.dao.GetTag(articleTag.ArticleID,model.STATE_OPEN)
	if err != nil{
		return nil, err
	}
	return &Article{
...
func (svc *Service)GetArticleList(param *ArticleListRequest,pager *app.Pager) ([]*Article,int,error) {
	articleCount,err := svc.dao.CountArticleByTagID(param.TagId,param.State)
	if err != nil{
		return nil, 0, err
	}
...
func (svc *Service)CreateArticle(param *CreateArticleRequest) error {
	...
	err = svc.dao.CreatedArticleTag(article.ID,param.TagId,param.CreatedBy)
	if err != nil{
		return err
	}
	return nil
